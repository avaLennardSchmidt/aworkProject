    ...
    try {
      if (operation.kind === "update") {
        if (!isOwnSchedule(operation.schedule, currentUser)) {
          results.push({
            operationId: operation.schedule.id,
            kind: operation.kind,
            success: false,
        ...
        await client.updateTaskSchedule(
          operation.schedule.id,
          buildUpdatePayload(change),
          currentUser.id,
        );
        results.push({
          operationId: operation.schedule.id,
      ...
      if (operation.payload.userId !== currentUser.id) {
        results.push({
          operationId: operation.tempId,
          kind: operation.kind,
          success: false,
          error: "Create payload does not belong to the selected planner user.",
        });
        continue;
      }
      ...
      const operationId =
        operation.kind === "create" ? operation.tempId : operation.schedule.id;
      results.push({
        operationId,
        kind: operation.kind,
        success: false,
