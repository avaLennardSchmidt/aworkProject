...
export function ManualBlockerEditModal({
  group,
  currentUser,
  onBack,
  onClose,
  onPreview,
}: ManualBlockerEditModalProps) {
    ...
    () =>
      existingRows.map((row) => {
        const schedule = group.schedules.find(
          (candidate) => candidate.id === row.scheduleId,
        );
        if (!schedule)
          return {
            row,
        ...
        return {
          ...
          changed,
          rowError: changed
            ? validateExistingRow(schedule, row, currentUser)
            : "",
        };
      }),
  ...
  const newViews = useMemo(
    () => newRows.map((row) => ({ row, rowError: validateNewRow(row) })),
    [newRows],
  );
  const operationCount =
    existingViews.filter((view) => view.changed && !view.rowError).length +
    newViews.filter((view) => !view.rowError).length;
  const invalidCount =
    existingViews.filter((view) => view.rowError).length +
    newViews.filter((view) => view.rowError).length;

  function updateExistingRow(
    scheduleId: string,
    patch: Partial<ExistingRowState>,
  ) {
    setError("");
    setExistingRows((rows) =>
      rows.map((row) =>
        row.scheduleId === scheduleId ? { ...row, ...patch } : row,
      ),
    );
  }
  ...
  function updateNewRow(tempId: string, patch: Partial<NewRowState>) {
    setError("");
    setNewRows((rows) =>
      rows.map((row) => (row.tempId === tempId ? { ...row, ...patch } : row)),
    );
  }
    ...
    existingViews.forEach((view) => {
      ...
      if (view.row.remove) {
        operations.push({
          ...
          beforeMinutes: calculateDurationMinutes(
            view.schedule.start,
            view.schedule.end,
          ),
        });
      ...
      operations.push({
        ...
        afterMinutes: calculateDurationMinutes(
          updated.newStartIso,
          updated.newEndIso,
        ),
      });
  ...
  return (
    <ModalShell
      labelledBy="manual-edit-title"
      dialogClassName="modal modal-wide manual-edit-modal"
      onClose={onClose}
    >
      ...
      <div className="preview-summary">
        ...
        <span>
          {existingRows.filter((row) => row.remove).length} zum Ausplanen
        </span>
      </div>
      ...
      <div className="manual-edit-list">
        {existingViews.map((view) => {
          ...
          if (!schedule)
            return (
              <div
                key={view.row.scheduleId}
                className="manual-edit-row has-error"
              >
          ...
          const rowClassName = [
            "manual-edit-row",
            view.changed ? "is-changed" : "",
            view.rowError ? "has-error" : "",
            view.row.remove ? "is-removing" : "",
          ]
            .filter(Boolean)
            .join(" ");
          return (
            <div key={schedule.id} className={rowClassName}>
              ...
              <div className="manual-row-fields">
                <label>
                  ...
                  <DatePickerInput
                    ...
                    onChange={(val) =>
                      updateExistingRow(schedule.id, { date: val })
                    }
                  />
                </label>
                <label>
                ...
                <button
                  type="button"
                  className="ghost-button manual-reset-button"
                  disabled={!view.changed}
                  onClick={() => resetExistingRow(schedule.id)}
                >
                  Zurücksetzen
                </button>
                <button
                  ...
                  title={
                    view.row.remove ? "Blocker behalten" : "Blocker ausplanen"
                  }
                  aria-label={
                    view.row.remove ? "Blocker behalten" : "Blocker ausplanen"
                  }
                  ...
                  onClick={() =>
                    updateExistingRow(schedule.id, { remove: !view.row.remove })
                  }
                >
              ...
              {view.rowError ? (
                <div className="manual-row-error">{view.rowError}</div>
              ) : null}
            </div>
          );
        })}
            ...
            >
              ...
              <div className="manual-row-fields">
                <label>
                  ...
                  <DatePickerInput
                    value={row.date}
                    onChange={(val) => updateNewRow(row.tempId, { date: val })}
                  />
                </label>
                <label>
                  Start
              ...
              {rowError ? (
                <div className="manual-row-error">{rowError}</div>
              ) : null}
            </div>
          );
        })}
      ...
      <div className="modal-actions">
        ...
        >
          {operationCount} Änderung{operationCount === 1 ? "" : "en"} vorschauen
        </button>
      </div>
    </ModalShell>
  );
...
function validateExistingRow(
  schedule: ScheduleGroup["schedules"][number],
  row: ExistingRowState,
  currentUser: AworkUser,
): string {
...
function buildExistingWindow(row: ExistingRowState): {
  newStartIso: string;
  newEndIso: string;
} {
  const payload = buildCreatePayload(
    {
      tempId: row.scheduleId,
      date: row.date,
...
function validateNewRow(row: NewRowState): string {
  if (!row.date || !row.startTime || !row.endTime)
    return "Datum, Start und Ende eingeben.";
  try {
